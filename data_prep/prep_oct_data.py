...
if __name__ == '__main__':
    ...
    if use_mini_dataset:
        ...
        areas_to_keep = {lbl: list(set([fm.parts[-2].split('_')[-1] for fm in files])) for (lbl, files) in
                         raw_file_per_label.items()}
        areas_to_keep = {lbl: random.sample(areas, 4) for (lbl, areas) in areas_to_keep.items()}
        raw_files = [fm for (lbl, areas) in areas_to_keep.items() for area in areas for fm in raw_mat_files if
                     area + "_" in fm.__str__() and lbl in fm.__str__()]
    ...
    # Create map df
    print("Creating the mapping dataframes...") # Use file_stats
    ...
    for split in sub_sets:
        ...
        df_map_split.to_csv(dataset_root.joinpath(map_df_path), index=False)
